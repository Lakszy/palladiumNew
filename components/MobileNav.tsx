  ...
  return (
    <div className="w-12 h-12 title-text flex">
      ...
      <Sidebar
        ...
        closeIcon={<RxCross2 className="text-[#88e273]"/>}
      >
        <div
              ...
              <Link legacyBehavior key={menuItem.id} href={menuItem.link}>
                <a
                  className={`cursor-pointer text-xl menu flex min-w-[200px] items-center gap-x-3 rounded-lg p-2 ${
                    isMenuSelected(menuItem.id) ? "bg-[#88e273] text-black" : "text-gray-500"
                  }`}
      ...
      <div className="ml-[10px] z-10 cursor-pointer" onClick={() => setVisible(!visible)}>
        {visible ? (
          <GiHamburgerMenu size={42} className="text-[#88e273]" />
        ) : (
          <GiHamburgerMenu size={42} className="text-[#88e273] mt-[0.5rem]" />
        )}
      </div>
    </div>
  );
