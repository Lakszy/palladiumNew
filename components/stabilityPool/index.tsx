...
export const StabilityPool = () => {
	...
	useEffect(() => {
		console.log(isLoading, 'Loading', isSuccess, 'isSuccess');
		if (isLoading) {
			setIsModalVisible(false); // Hide modal when loading
	...
	// Listen for changes in the hash state
	useEffect(() => {
		if (hash) {
			setIsModalVisible(false);
		}
...
return (
	<div className="grid  bg-[#3b351b] items-start h-64 gap-2 mx-auto border border-yellow-400 p-5">
		...
		<div className="flex w-full gap-x-4 md:gap-x-6  mt-2">
			<Button disabled={!isConnected || isDataLoading} className={`text-xs md:text-lg border-2 border-yellow-900 body-text ${isDataLoading ? 'cursor-not-allowed' : ''}`} style={{ backgroundColor: "#3b351b", borderRadius: "0" }} onClick={() => handlePercentageClick(25)}>25
				%</Button>
		...
		<Dialog visible={loadingModalVisible} onHide={() => setLoadingModalVisible(false)}>
			<div className="waiting-container bg-white">
				...
				{isSuccess && (
					<Button className="p-button-rounded p-button-text" onClick={() => setLoadingModalVisible(false)}>
						Close
					</Button>
				)}
			</div>
