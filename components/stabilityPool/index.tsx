	...
	return (
		<div className="grid  bg-[#3b351b] items-start h-64 gap-2 mx-auto border border-yellow-400 p-5">
			<div className="">
				...
				<span className="ml-[45%] body-text text-yellow-300 font-medium balance ">
					{isDataLoading ? (
						<div className="text-left w-full  -mt-6 h-2">
							<div className="hex-loader"></div>
						</div>
					) : (
						< span className="whitespace-nowrap body-text"><span className="text-gray-400 body-text">
							Wallet:{" "}
						</span>{Math.trunc(Number(pusdBalance) * 100) / 100} PUSD</  span>
					)}
				</span>
			</div>
			...
			{isConnected ? (
				<div className="">
					<button
						...
						className={`mt-2 text-black text-md font-semibold w-full border border-black h-10 body-text border-none ${isDataLoading ? 'cursor-not-allowed opacity-50' : ''
							}`}
