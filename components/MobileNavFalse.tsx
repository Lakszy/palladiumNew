      ...
      <Sidebar visible={visible} onHide={() => setVisible(false)}>
        <div className={`sidebar bg-red-90 h-full font-mono font-extrabold  w-full grid-rows-[max-content_fr_max-content] text-white`} style={{ backgroundColor: "#2c2819" }}>
          <div className="flex items-left align-text- gap-x-1 ">
            <Link href="/">
              <Image src={logo} alt="Logo" className=" ml-2 -mt-1 mb-3 w-40" />
            </Link>
          </div>
          <nav className="flex flex-col gap-y-2 px-4">
            {menuItems.map((menuItem) =>
              <a
                key={menuItem.id}
                className={`opacity-50 cursor-not-allowed  text-xl menu flex min-w-[200px] items-center gap-x-3 rounded-lg p-2 ${isMenuSelected(menuItem.id)
                  ? "text-gray-500"
                  : "text-gray-500"
                  }`}
                onClick={() => handleMenuClick(menuItem.id)}
              >
                <div className={`cursor-not-allowed  menu flex items-center gap-x-3 rounded-full p-2 isMenuSelected(menuItem.id) ? "text-black" : " text-white"    `}>
